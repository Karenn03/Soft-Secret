...
export default function NavbarHome() {
    ...
    return (
        <>
            <nav
                className={`fixed top-0 left-0 z-50 bg-linear-to-r from-[#39A900] to-lime-600 backdrop-blur-sm w-full md:text-sm md:border-none ${state ? "shadow-lg rounded-b-xl md:shadow-none" : ""
                    }`}
            >
                    ...
                    >
                        <ul className="items-center space-y-6 md:flex md:space-x-1 md:space-y-0">
                            <li className="flex-1 items-center justify-end gap-x-6 space-y-3 md:flex md:space-y-0">
                                <Link
                                    href="/auth/signUp"
                                    className="block py-2 px-4 font-semibold text-base text-center text-white bg-white/30 rounded-lg shadow md:inline transition-all duration-300 hover:shadow-lg hover:bg-white/40 hover:-translate-y-1"
                                >
                                    Inicia sesión
                                </Link>
                            </li>
